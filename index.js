...
// Apply Z compensation to G-code with move subdivision for smooth curves
function applyZCompensation(gcodeContent, mesh, gridParams, referenceZ) {
  ...
  let currentGMode = 'G1'; // Track G0/G1/G2/G3
    ...
    if (isLinearMove && hasXY && isAbsolute) {
      ...
      if (distance > segmentLength) {
        ...
        for (let i = 1; i <= segments; i++) {
          ...
          const segZ = currentZ + dz * t; // Linear Z interpolation from original

          // Apply mesh compensation
            ...
            for (let r = 0; r < rows && !stopProbing; r++) {
              for (let c = 0; c < cols && !stopProbing; c++) {
                ...
                if (isFirstPoint) {
                  ...
                  // Subsequent points: smart lateral probing
                ...
                try {
                  ...
                  if (prb && prb.success) {
                    ...
                    // Retract 1mm after probe - just enough to clear probe trigger
                    // If lateral move hits surface, we'll bounce 5mm then
